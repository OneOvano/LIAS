...
class test_spectra(unittest.TestCase):
    ...
    def test_spec(self):
        ...
        target = sp.Object_Spectrum - spectrum
        assert (sp == target).all
...
class test_tools(unittest.TestCase):
    ...
    def test_calc_cont(self):
        ...
        wln = 6550
        target = 1.
        assert (lias.Tools.calc_cont(x, y, wln, 1) - target) < EPS

    def test_masc(self):
        x = np.array([1., 2., 3.])
        y = np.array([0., 1., 0.])
        target = 2.
        assert (lias.Tools.calc_masc(x, y) - target) < EPS
    ...
    def test_intense(self):
        ...
        target = 0.25
        assert np.abs(lias.Tools.calc_int(x, y, cnt)/target-1) < EPS

    def test_gauss(self):
        x = np.linspace(-1, 1, 100)
        target = (1., 0., 0.5)
        y = lias.Tools.gauss_function(
            x,
            target[0],
            target[1],
            target[2]
